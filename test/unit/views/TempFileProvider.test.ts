...
describe('TempFileProvider', () => {
  ...
  it('provideTextDocumentContent returns empty string for an unknown URI', () => {
    const provider = new TempFileProvider('claude-vscode-left');
    const uri = makeUri('claude-vscode-left', '/test.ts');
  ...
  it('makeUri returns a URI with the provider scheme', () => {
    const provider = new TempFileProvider('claude-vscode-right');
    provider.makeUri('/foo/bar.ts');
    expect(mockVscode.Uri.from).toHaveBeenCalledWith({ scheme: 'claude-vscode-right', path: '/foo/bar.ts' });
  });

  it('stores multiple URIs independently', () => {
    const provider = new TempFileProvider('claude-vscode-left');
    const uri1 = makeUri('claude-vscode-left', '/a.ts');
    const uri2 = makeUri('claude-vscode-left', '/b.ts');
  ...
  it('scheme is accessible as a property', () => {
    const provider = new TempFileProvider('claude-vscode-right');
    expect(provider.scheme).toBe('claude-vscode-right');
  });
});
