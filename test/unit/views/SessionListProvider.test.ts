...
import { SessionListProvider } from '../../../src/views/SessionListProvider';
...
describe('SessionListProvider', () => {
  ...
  function makeProvider() {
    return new SessionListProvider(
  ...
  it('VIEW_ID is the expected view identifier', () => {
    expect(SessionListProvider.VIEW_ID).toBe('clawdVSCodeSessionsList');
  });
  ...
  it('resolveWebviewView() builds HTML with isSessionListOnly=true', () => {
    ...
    expect(htmlBuilder.build).toHaveBeenCalledWith(
      wv.webview,
      expect.objectContaining({ isSessionListOnly: true }),
    );
  });
  ...
  it('dispose event unregisters from ViewManager and marks inactive', () => {
    const provider = makeProvider();
    const wv = makeMockWebviewView();
    provider.resolveWebviewView(wv as never, {} as never, {} as never);
