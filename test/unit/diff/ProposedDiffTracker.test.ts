...
describe('ProposedDiffTracker', () => {
  ...
  it('isProposedDiff returns false for untracked URI', () => {
    expect(tracker.isProposedDiff(makeUri('clawd-vscode-right:///test.ts'))).toBe(false);
  });

  it('trackDiff makes isProposedDiff return true', () => {
    const uri = makeUri('clawd-vscode-right:///test.ts');
  ...
  it('tracks multiple URIs independently', () => {
    const a = makeUri('clawd-vscode-right:///a.ts');
    const b = makeUri('clawd-vscode-right:///b.ts');
    tracker.trackDiff(a);
  ...
  it('updateContextKey sets context to true when active editor is a tracked diff', () => {
    const uri = makeUri('clawd-vscode-right:///test.ts');
    tracker.trackDiff(uri);
    tracker.updateContextKey(makeEditor('clawd-vscode-right:///test.ts'));
    expect(setContext).toHaveBeenCalledWith('clawd-vscode.viewingProposedDiff', true);
  });
  ...
  it('updateContextKey sets context to false when active editor is not a tracked diff', () => {
    ...
    expect(setContext).toHaveBeenCalledWith('clawd-vscode.viewingProposedDiff', false);
  });
